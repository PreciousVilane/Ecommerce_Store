...
class Tweet:
    _instance = None
    CONSUMER_KEY = "Ewj5fWfKWmnWT3TUwvRAQop49"
    CONSUMER_SECRET = "Q9Sz1DIsA4Hatbm6LcOrbwIkT9tn5QoB1we0X1LwQ8zVWAO6HK"
    ...
    def tweet_new_store(store):
        ...
        try:
            tweet_client = Tweet()
            tweet_text = (
                f"New store open on Grabsomore!\n\n" f"Store: {store.name}\n"
                f"Description: {store.description}\n"
            )
            tweet_data = {"text": tweet_text}
            tweet_client.make_tweet(tweet_data)
        except Exception as e:
            print(f"Twitter error when posting new store: {e}")
    ...
    def tweet_new_product(product):
        ...
        try:
            ...
            print(f"Twitter error when posting new product: {e}")
